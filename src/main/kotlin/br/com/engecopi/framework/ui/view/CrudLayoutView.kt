...
abstract class CrudLayoutView<C: EntityVo<*>, V: CrudViewModel<*, *, C>>: LayoutView<V>() {
  ...
  fun buildNewForm(operation: CrudOperation,
                   ...
                   operationButtonClickListener: () -> Unit): CrudForm<C> {
  ...
  protected fun gridSelectionChanged() {
    updateButtons()
    val domainObject = grid.asSingleSelect()
      ?.value

    if(domainObject != null) {
      val form = buildNewForm(READ, domainObject, true, {}) {
        grid.asSingleSelect()
          ?.clear()
      }
  ...
  private fun addButtonClicked() {
    try {
      val domainObject = viewModel.crudClass.objectInstance ?: return
...
class CrudForm<C: EntityVo<*>>(val operation: CrudOperation,
                               ...
                               cancelButtonClickListener: () -> Unit,
                               operationButtonClickListener: () -> Unit,
  ...
  init {
    ...
    layoutForm(this)
    mainLayout.addComponentsAndExpand(formLayout)

    updateButtons()

    mainLayout.addComponent(footerLayout)
    mainLayout.setComponentAlignment(footerLayout, BOTTOM_RIGHT)
  ...
  private fun buildFooter(operation: CrudOperation,
                          cancelButtonClickListener: () -> Unit,
                          operationButtonClickListener: () -> Unit): Layout {
  ...
  private fun buildOperationButton(operation: CrudOperation, clickListener: () -> Unit): Button {
    val button = Button(buttonCaptions[operation], buttonIcons[operation])

    button.addStyleName(buttonStyleNames[operation])
    button.addClickListener {_ ->
      val validate = binder.validate()
      if(validate.isOk) clickListener()
  ...
  private fun buildCancelButton(clickListener: () -> Unit): Button? {
    return Button("Cancela") {_ -> clickListener()}
  }
}
