...
abstract class NotaVo(val tipo: TipoMov, private val abreviacaoNota: String): EntityVo<ItemNota>() {
  ...
  private val notaProdutoSaci = ArrayList<NotaSaci>()

  private fun carregaProdutoSaci(numero: String?): List<NotaSaci> {
    numero ?: return emptyList()
    return if(entityVo == null) when(tipo) {
      SAIDA   -> Nota.findNotaSaidaSaci(numero)
      ENTRADA -> Nota.findNotaEntradaSaci(numero)
    }.filter {
  ...
  private fun atualizaNota() {
    notaProdutoSaci.clear()
    notaProdutoSaci.addAll(carregaProdutoSaci(numeroNF))
